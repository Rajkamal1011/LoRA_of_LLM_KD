...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.LoRA_c_attn = LoRALinear(config.n_embd, 3*config.n_embd, LoRA_rank=config.LoRA_rank) # Check out once LoRALinear(config.n_embd, 3*config.n_embd, LoRA_rank=config.LoRA_rank)
        self.LoRA_c_proj = LoRALinear(config.n_embd, config.n_embd, LoRA_rank=config.LoRA_rank)
    ...
    def forward(self, x):
        ...
        #LoRA_attn_out = self.LoRA_c_attn(attn_out) + attn_out # Check this once self.LoRA_c_attn(x) + self.c_attn(x)
        LoRA_attn_out = self.LoRA_c_attn(x) + self.c_attn(x)
        ...
        q, k, v  = LoRA_attn_out.split(self.n_embd, dim=2)
        ...
        #NEW CODE Line
        #LoRA_proj_out = self.LoRA_c_proj(proj_out) + proj_out #Again check this once self.LoRA_c_proj(y) + proj_out 
        LoRA_proj_out = self.LoRA_c_proj(y) + self.c_proj(y)

        # TODO: send y through the auxiliary c_proj and add them back to proj_out
        # y = self.resid_dropout(proj_out)

        y = self.resid_dropout(LoRA_proj_out)
        return y
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        #self.LoRA_c_fc = LoRALinear(4*config.n_embd, 4*config.n_embd, LoRA_rank = config.LoRA_rank) #Check this once LoRALinear(config.n_embd, 4 * config.n_embd, LoRA_rank = config.LoRA_rank)
        self.LoRA_c_fc = LoRALinear(config.n_embd, 4*config.n_embd, LoRA_rank = config.LoRA_rank) 
    ...
    def forward(self, x):
        ...
        #x = self.c_fc(x) #Comment this one out
        #LoRA_fc = self.LoRA_c_fc(x) + x #Check this once self.LoRA_c_fc(x) + self.c_fc(x)
        LoRA_fc = self.LoRA_c_fc(x) + self.c_fc(x)
        x = self.gelu(LoRA_fc)
        # x = self.gelu(x)
        x = self.c_proj(x)
        # LoRA_proj = self.LoRA_c_proj(x) + x
        LoRA_proj = self.LoRA_c_proj(x) + x
        x = self.dropout(LoRA_proj)
        # x = self.dropout(x)
        return x
...
class LoRALinear(nn.Module):
    #Adding New CODE
    def __init__(self, in_features, out_features, LoRA_rank=1):
        super(LoRALinear, self).__init__()
        self.L_matrix = nn.Parameter(torch.randn(in_features, LoRA_rank))
        self.R_matrix = nn.Parameter(torch.randn(LoRA_rank, out_features))
        self.dropout = nn.Dropout(0,1)
        self.reset_parameters()
    
    def forward(self, input):
        return torch.matmul(torch.matmul(input, self.L_matrix), self.R_matrix)

    def reset_parameters(self):
        nn.init.xavier_uniform_(self.L_matrix)
        nn.init.xavier_uniform_(self.R_matrix)
...
class DistilRNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.rnn = nn.RNN(768, 1536, 1, batch_first=True)
